...
val targetedCompactedFileSizeInMB = 64
//TODO RM: target output file size


// COMMAND ----------

//Function to calculate # of output files
//accepts file path and target output file size
val calcOutputFileCount = (srcDataFile: String, targetedCompactedFileSizeInMB: Int) => {
  val estFileCount = Math.floor((fs.getContentSummary(new Path(srcDataFile)).getLength * 3/16) / (targetedCompactedFileSizeInMB * 1024 * 1024)).toInt
  if(estFileCount == 0) 1 else estFileCount
}

// COMMAND ----------

//Canonical column list for yellow taxi
//We will use to across schemas and years to get a consistently ordered column list
//These are actual columns names in the header of source data
...
for (j <- 2017 to 2017)
  {
    ...
    for (i <- 1 to 12) 
    {
      ...
      if(j > 2008 && j < 2015){
        ...
        //This step is to order all columns to align with the canonical schema for yellow taxi
        ...
        taxiCanonicalDF.coalesce(calcOutputFileCount(srcDataFile, targetedCompactedFileSizeInMB)).write.parquet(destDataDir)
...
//Temporary stub - for us to use during testing
