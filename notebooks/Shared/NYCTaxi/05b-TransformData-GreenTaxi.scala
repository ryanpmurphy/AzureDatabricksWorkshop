...
// MAGIC REFRESH TABLE nyc_db.trips_green_raw_prq;
// MAGIC 
...
val sqlDF = spark.sql("""
  ...
  left join nyc_db.refdata_vendor_lookup v on t.vendor_id = v.vendor_id
...
for (j <- 2014 to 2017)
  {
    ...
    {    
      ...
      val alterTableSql = "ALTER TABLE nyc_db.trips_green_transformed_prq ADD IF NOT EXISTS PARTITION (trip_year=" + j + ",trip_month=" + "%02d".format(i) + ") LOCATION '" + destDataDir.dropRight(1) + "'"
      sql(alterTableSql)
      //println(alterTableSql)
      
       //Refresh table
       sql("REFRESH TABLE nyc_db.trips_green_transformed_prq")
    }
  }
sql("ANALYZE TABLE nyc_db.trips_green_transformed_prq COMPUTE STATISTICS")
